...
`lcm install` writes config, registers MCP, installs the `/memory` skill and `lcm.md`, verifies the daemon, and, when `codex` is on PATH, installs the Codex hooks globally. It reports one outcome per harness and exits non-zero on any failure; `--dry-run` writes nothing.
