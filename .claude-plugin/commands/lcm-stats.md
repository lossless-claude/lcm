...
If `lcm_stats` is unavailable, run `lcm stats` via Bash and display the output verbatim. If `lcm` is not on PATH (marketplace install), use `node "${CLAUDE_PLUGIN_ROOT}/lcm.mjs" stats -v` instead.
