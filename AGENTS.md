...
| cutting a release | [docs/releasing.md](docs/releasing.md) | `.github/workflows/publish.yml`, `version-pr.yml` |
...
- Repository only: `.claude/`, `.agents/`, `.github/`, `scripts/`, `test/`, `tools/`, `plans/`, `WORKFLOW.md`, and every doc about developing lcm rather than using it.
...
- `main` is the only long-lived branch; PRs target it, and a push to `main` that changes `package.json` publishes.
...
CI runs the same three. `claude plugin validate .claude-plugin/plugin.json` walks the plugin's own agents, skills and commands; run it after touching any of them.
