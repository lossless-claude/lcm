...
**Dedup rule:** the module claims its session. At `session.start` it writes `<tmpdir>/lcm-claim-<session_id>.json` containing `{ sessionId, ts }`, awaited before the hook returns so the file is there before the first prompt. `lcm post-tool`, `lcm user-prompt` and `lcm session-snapshot` then exit without recording or printing anything when `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` **and** that file names their session (`functionHooksOwnSession` in `src/hooks/session-claim.ts`); otherwise every event would land twice and the model would read the memory context twice.
